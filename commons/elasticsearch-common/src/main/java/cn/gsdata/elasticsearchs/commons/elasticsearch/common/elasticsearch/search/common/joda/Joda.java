...
public class Joda {
    ...
    public static class EpochTimePrinter implements DateTimePrinter {
        ...
        @Override
        public void printTo(StringBuffer stringBuffer, long instant, Chronology chronology, int displayOffset, DateTimeZone dateTimeZone, Locale locale) {
            if (hasMilliSecondPrecision) {
                stringBuffer.append(instant - displayOffset);
            } else {
                stringBuffer.append((instant - displayOffset) / 1000);
            }
        }

        @Override
        public void printTo(Writer writer, long instant, Chronology chronology, int displayOffset, DateTimeZone dateTimeZone, Locale locale) throws IOException {
            if (hasMilliSecondPrecision) {
                writer.write(String.valueOf(instant - displayOffset));
            } else {
                writer.append(String.valueOf((instant - displayOffset) / 1000));
            }
        }
        ...
        public void printTo(Writer writer, ReadablePartial readablePartial, Locale locale) throws IOException {
            if (hasMilliSecondPrecision) {
                writer.append(String.valueOf(getDateTimeMillis(readablePartial)));
            } else {
                writer.append(String.valueOf(getDateTimeMillis(readablePartial) / 1000));
            }
        }
